...
namespace Flutter.Support.Dapper.Extensions
{
    /// <summary>Dapper extensions.
    /// </summary>
    public static class SqlDapperExtensions
    {
        ...
        public static async Task BatchInsertAsync(this IDbConnection connection, IEnumerable<dynamic> datas, string table, int BatchSize = 1000, int BulkCopyTimeout = 60, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var properties = GetPropertyInfos(datas.First() as object);
            ...
            var copy = transaction != null ? new SqlBulkCopy(sqlconnection, SqlBulkCopyOptions.Default, transaction as SqlTransaction) : new SqlBulkCopy(sqlconnection);
            ...
            using (copy)
            {
                ...
                await copy.WriteToServerAsync(datatable.CreateDataReader());
        ...
        /// <param name="commandTimeout"></param>
        /// <returns></returns>
        public static Task<long> InsertGeomAsync(this IDbConnection connection, string coordinateRange, dynamic data, string table, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            var obj = data as object;
            var properties = GetProperties(obj);
            var columns = string.Join(",", properties);
            var values = string.Join(",", properties.Select(p => "@" + p));
            var sql = string.Format("insert into {0} (coordinateRange, {1}) values (geometry::STGeomFromText{3},{2})", table, columns, values, coordinateRange);
        ...
        private static List<PropertyInfo> GetPropertyInfos(object obj)
        {
            ...
            if (_paramCache.TryGetValue(obj.GetType(), out List<PropertyInfo> properties))
            {
